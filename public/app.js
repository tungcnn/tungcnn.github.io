...
$('#storeregister').on('click', function() {
    const name = $('#usernameP').val();
    const password = $('#passwordPP').val();
    const confirmPassword = $('#confirm-passwordP').val();
    const lat = $('#lat').val();
    const lon = $('#lon').val();
...
$('#loginstore').on('click', () => {
    const user = $('#userP').val();
    const password = $('#passwordP').val();
    $.post(`${API_URL}/storeauthenticate`, { user, password })
    .then((response) =>{
        if (response.success) {
